...
Link to repository:
\
https://github.com/andreigraham/BIS15L_W26_andreigraham.git
\
\
\
...
\
```{r}
anolis<- read_csv("midterm1/data/anolis_dat.csv")%>% clean_names()
```
\
\
\
...
#anolis$habitat<- as.factor(anolis$habitat)
#anolis$island<- as.factor(anolis$island)
...
```{r}
 # ggplot(data=anolis,
 #        mapping=aes(x=island))+geom_bar()+
#  labs(x="island",
 #      y= "number of samples",
#       title = "number of samples per island")

#sampling is unequal across islands, with Cuba having the most samples and Jamacia having the least
```
\
\
\
...
```{r}
#ggplot(data=anolis,
#         mapping=aes(x=habitat))+geom_bar()+
#  labs(x="type of habitat",
#       y= "number of samples",
 #      title = "number of samples per habitat")

#The number of samples varies per habitat, with trunk-ground having te highest number of samples and twig having the lowest
```
\
\
\
...
```{r}
ggplot(data=anolis,
         mapping=aes(x=island, y=hindlimb_length_mm))+geom_boxplot()
  labs(x="island",
       y= "hindlimb length in mm",
       title = "hindlimb length in mm per island")
  
  #Hispaniola has the widest range of hindlimb length, and also more frequently have anolis with both longer and shorther hindlimbs
```
\
\
\
...
\
```{r}
ratio_of_hindlimb_to_body<- c(anolis$hindlimb_length_mm/anolis$body_length_mm)

ratio_of_tail_to_body<- c(anolis$tail_length_mm/anolis$body_length_mm)

anolis<- anolis
#colnames(anolis)<- ((colnames(anolis))& ratio_of_hindlimb_to_body& ratio_of_tail_to_body)

#anolis%>%
#(anolis$species, anolis$habitat, anolis$hindlimb_length_mm, anolis$tail_length_mm, anolis$island, anolis$toepad_lamellae_count, anolis$body_length_mm, anolis$ratio_of_hindlimb_to_body, anolis$ratio_of_tail_to_body)
```
\
\
\
...
```{r}
ggplot(data=anolis,
         mapping=aes(x=habitat, y=ratio_of_hindlimb_to_body))+geom_boxplot()
  labs(x="island",
       y= "hindlimb to body ratio",
       title = "hindlimb ratio per habitat")
  
  #This is different from the previous graph since it shows habitat instead of island, there can be many habitats on one island.
```
\
\
\
...
#anolis%>%
#(anolis$habitat)%>%
#summarise(mean(toepad_lamellae_count, na.rm=T),
 #total=n())
#anolis%>%
#  select(habitat =="twig")%>%
#mean(anolis$toepad_lamellae_count)
```
\
\
\
