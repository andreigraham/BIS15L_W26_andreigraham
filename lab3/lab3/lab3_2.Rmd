...
We only use `tibble() when working in the tidyverse. The equivalent command in Base R is
...
```{r}
fish<-tibble(Gaeta_etal_CLC_data)

read_csv("data/Gaeta_etal_CLC_data.csv")
```
...
```{r}
summary(Gaeta_etal_CLC_data)
```

`glimpse()` is another useful summary function.
```{r}
glimpse(Gaeta_etal_CLC_data)
```
...
```{r}
mammals<-tibble("mammal_lifehistories_v2.csv")
```
