...
```{r}
#gabon%>%
  #distinct(transect_id,land_use)%>%
  #ggplot(aes(x=land_use))+
  #geom_bar()

ggplot(data=gabon, 
       mapping=aes(x=land_use,y= transect_id))+
  geom_boxplot()+
  labs(x= "Land Use", y= "Transect ID")
...
ggplot(data=gabon, 
  ...
  geom_boxplot()+
  labs(x= "Land Use", y= "Vegetation Richness")

#Logging has the highest amount of vegetation richness between land usage. 
```

**7. For which land use type (national parks, logging, or neither) is average all species diversity the greatest?**
```{r}
#park has the greatest average diversity of species
```
...
gabon%>%
  group_by(land_use)%>%
  summarise(mean_div=mean(diversity_all_species, na.rm=T))%>%
...
gabon%>%
  ...
  mean(diversity_all_species, na.rm=T)
...
ggplot(data=gabon, 
  ...
  labs(x= "richness of vegetation", y= "richness of mammal species")
  
  #Yes, as the richness of vegetation increases, so does the richness of mammal species.
```
...
filter(gabon, between(veg_dbh,50,60)| veg_rich>15)
```
...
```{r}
filter(gabon, veg_rich>10& (rich_mammal_species>10))%>%
