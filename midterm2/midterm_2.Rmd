...
#simpleWarning: ‘lubridate’ namespace cannot be unloaded:
#  namespace ‘lubridate’ is imported by ‘janitor’ so cannot be unloaded
```
...
un_creat_ratio%>%
  ...
  group_by(year, popn)%>%
  summarise(n = n())%>%
  ggplot(aes(x = year, y = n, color = popn, group = popn))+
  geom_point()+
  geom_line()+ 
  labs( title = "number of fasting polar bears by year",
        x = "year",
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(data = un_creat_ratio, 
           aes(x=popn,
               y=un_creat_ratio, 
               fill = .data[[input$fill_var]]
               ))+
      geom_boxplot(alpha = 0.7, na.rm = T) +
      labs( title = " UN Creatine Ratio by Population",
            x = "population",
            y = "un creatine ratio")
  })
