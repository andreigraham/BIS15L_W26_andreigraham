...
```{r}
x <- 42 #read as x gets the value 42
...
```{r}
z=10 #this will work but create problems, use <- instead
```
...
```
```{r}
#option, command, i gives a new code chunk
```
...
my_experiment
#using tab to complete prevents syntax errors
```
...
```{r}
mean(c(2+8+2)) #c stnads for combine and creates a vector
```
...
# c is a vector and can be used to find the mean
```
...
mean_speed <- (venom_GT+veyron+chiron)/3
```
...
#is important when we work with plots
```
...
```{r}
mean(new_vector, na.rm = TRUE) #removes NA values and gives the values of numbers other than the missing data
```