...
elephants%>%
  ...
  summarise(across(where(is.numeric),
                         ~mean(.x, na.rm=T)))
#you must group by a categorical variable
```
...
elephants%>%
  ...
  summarise(mean_estimate_age_years=mean(estimated_age_years, na.rm=T),
            n=n(),
            .groups='keep')%>%
  arrange(sex)
```
...
```{r}
#the filter function needs a specific value to be assigned to it, being a character vector, you should add the quotation marks
elephants%>%
  filter(years_of_sample_collection== "1966-68")%>%
  group_by(sex)%>%
  summarise(mean_estimate_age_years=mean(estimated_age_years, na.rm=T),
            median_estimate_age_years=median(estimated_age_years, na.rm=T),
            sd_estimate_age_years=sd(estimated_age_years, na.rm=T))
```
...
msleep%>%
  group_by(vore)%>%
  summarise(mean_body_weight=mean(bodywt,na.rm=T),
            min_body_weight=min(bodywt,na.rm=T),
            max_body_weight=max(bodywt,na.rm=T))
```
