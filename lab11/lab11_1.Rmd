...
#linegraphs are best for change over time, there are some issues that can happen with time if there are any characters in it 
```
...
gap%>%
  filter(year==1935)%>%
  ggplot(aes(x=lex))+
  geom_histogram(color= "black", fill="pink3", bins=30)+
...
gap%>%
  filter(year==1935)%>%
  ggplot(aes(x=lex))+
  geom_density(color= "pink4", fill="pink3", alpha= 0.33)+
  labs(title= "Distribution of Life Expectancy")
```
...
gap%>%
  filter(year==1935)%>%
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)), color="pink4", fill= "pink3")+
...
gap%>%
  ...
  labs(title= "Life Expectancy Distribution Between 1930 & 1950",
       x= "Life Expectancy",
       y= NULL)
```
...
```{r}
income_cat2000%>%
  drop_na(income_category)%>% #gets rid of na's
