...
ggplot(data=penguins, #specifies the data
       mapping=aes(x=species, y=body_mass_g)) #we map the aesthetics
```
...
ggplot(data=penguins, #specifies the data
  ...
  geom_boxplot()#we map the aesthetics
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(na.rm=T)
```

To add a regression (best of fit) line, we add another layer.
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point()+ geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(mapping=aes(color=species))+geom_smooth(method=lm, se=T)
...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(mapping=aes(color=species, shape=island))+geom_smooth(method=lm, se=T)
```

But don't we want a title? We do this using the `labs()` function.
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(mapping=aes(color=species))+geom_smooth(method=lm, se=T)+ labs(title="body mass (g) vs. flipper length (mm)", x="body mass (g)")
```

### Practice
1. Is there a relationship between bill length and bill depth?
```{r}
ggplot(data = penguins, mapping=aes(x=bill_length_mm, y=bill_depth_mm))+geom_point(na.rm=T)+geom_point(mapping = aes(color=species))+geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(data = penguins,
       mapping = aes(x=island))+geom_bar(mapping = aes(fill=species), position="dodge")+labs(title="species of penguins per island")
```
